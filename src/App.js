...
import { MessageSquare, AlertTriangle, Clock, Upload as UploadIcon, } from 'lucide-react';
...
const checkAllFields = async (message) => {
    const results = {};
    // Check sender fields
    ...
    if (message.sender.company_details?.Founders) {
        await Promise.all(message.sender.company_details.Founders.map(async (founder, index) => {
            if (founder.owner) {
    ...
    if (message.receiver.Founders) {
        await Promise.all(message.receiver.Founders.map(async (founder, index) => {
            if (founder.owner) {
                const checkResult = await OfacChecker.checkName(founder.owner);
...
export default function App() {
    ...
    useEffect(() => {
        const fetchParsedFiles = async () => {
            try {
                const response = await axios.get('http://localhost:3001/api/parsed-swift-files');
                ...
                const loadedMessages = Object.entries(parsedFiles).map(([key, data]) => ({
                    ...
                    sender: {
                        ...
                        sdnStatus: 'pending', // Ensure 'sdnStatus' uses SdnStatusType
    ...
    const handleFilterChange = (filters) => {
        const filtered = messages.filter(message => {
            if (filters.search) {
                ...
                const searchMatch = message.sender.name.toLowerCase().includes(searchLower) ||
            ...
            if (filters.dateFrom && new Date(message.date) < new Date(filters.dateFrom))
                return false;
            if (filters.dateTo && new Date(message.date) > new Date(filters.dateTo))
                return false;
            ...
            if (filters.bankName) {
                ...
                const bankMatch = message.receiver.bankName?.toLowerCase().includes(bankNameLower) ||
    ...
    const handleStoreChecks = (messageId, checks) => {
        setMessageChecks(prev => ({
            ...prev,
            [messageId]: checks
        }));
    };
    const handleUpload = async (messageText, comments) => {
        try {
    ...
    const handleAddBlacklistEntry = (entry) => {
        const newEntry = {
            ...entry,
            id: crypto.randomUUID(),
    ...
    const handleUpdateBlacklistEntry = (id, entry) => {
        setBlacklist(prev => prev.map(item => item.id === id
    ...
    const handleNotesChange = (id, notes) => {
        setMessages(prev => prev.map(msg => (msg.id === id ? { ...msg, notes } : msg)));
    };
    return (_jsxs("div", { className: "min-h-screen flex flex-col bg-gray-100 dark:bg-gray-900 transition-colors", children: [_jsx(Navbar, { currentPage: currentPage, onNavigate: setCurrentPage, isDark: isDark, onToggleTheme: () => setIsDark(!isDark) }), _jsx("main", { className: "flex-grow container mx-auto px-4 py-8", children: currentPage === 'dashboard' ? (_jsxs(_Fragment, { children: [_jsxs("div", { className: "grid grid-cols-1 md:grid-cols-3 gap-6 mb-8", children: [_jsx(DashboardCard, { title: "Total Messages", value: filteredMessages.length, icon: MessageSquare }), _jsx(DashboardCard, { title: "Flagged Messages", value: filteredMessages.filter(m => m.status === 'flagged').length, icon: AlertTriangle }), _jsx(DashboardCard, { title: "Processing", value: filteredMessages.filter(m => m.status === 'processing').length, icon: Clock })] }), _jsx("div", { className: "bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 mb-6", children: _jsxs("div", { className: "flex items-center space-x-4", children: [_jsx("div", { className: "flex-grow", children: _jsx(DashboardFilters, { onFilterChange: handleFilterChange }) }), _jsxs("button", { onClick: () => setIsUploadModalOpen(true), className: "flex-shrink-0 h-10 flex items-center px-4 bg-[#008766] text-white rounded-lg hover:bg-[#007055] dark:bg-[#007055] dark:hover:bg-[#006045] transition-colors", children: [_jsx(UploadIcon, { className: "w-4 h-4 mr-2" }), "New Message"] })] }) }), _jsx(MessageList, { messages: filteredMessages, onViewMessage: handleViewMessage, onDeleteMessage: handleDeleteMessage, onStatusChange: handleStatusChange }), _jsx(UploadModal, { isOpen: isUploadModalOpen, onClose: () => setIsUploadModalOpen(false), onUpload: handleUpload }), selectedMessage && (_jsx(MessageDetailsModal, { message: selectedMessage, isOpen: !!selectedMessage, onClose: () => setSelectedMessage(null), onStatusChange: handleStatusChange, onNotesChange: handleNotesChange, blacklist: blacklist, savedChecks: messageChecks[selectedMessage.id], onStoreChecks: (checks) => handleStoreChecks(selectedMessage.id, checks) }))] })) : currentPage === 'entity-search' ? (_jsx(EntitySearch, {}) /* Render EntitySearch component */) : currentPage === 'reports' ? (_jsx(Reports, { messages: messages })) : currentPage === 'sdn-list' ? (_jsx(SDNList, { isDark: isDark })) : (_jsx(BlacklistManager, { entries: blacklist, onAddEntry: handleAddBlacklistEntry, onUpdateEntry: handleUpdateBlacklistEntry, onDeleteEntry: handleDeleteBlacklistEntry })) }), _jsx(Footer, {})] }));
}
